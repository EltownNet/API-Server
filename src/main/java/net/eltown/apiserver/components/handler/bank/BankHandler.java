...
public class BankHandler extends Handler<BankProvider> {
    ...
    public BankHandler(final Server server) {
        super(server, "BankHandler", new BankProvider(server));
        ...
        this.getServer().getExecutor().execute(() -> {
            this.getTinyRabbitListener().receive(delivery -> {
                ...
                switch (BankCalls.valueOf(delivery.getKey().toUpperCase())) {
                    case REQUEST_INSERT_LOG -> this.getProvider().insertBankLog(d[1], d[2], d[3]);
                    case REQUEST_WITHDRAW_MONEY -> this.getProvider().withdrawMoney(d[1], Double.parseDouble(d[2]));
                    case REQUEST_DEPOSIT_MONEY -> this.getProvider().depositMoney(d[1], Double.parseDouble(d[2]));
                    case REQUEST_SET_MONEY -> this.getProvider().setMoney(d[1], Double.parseDouble(d[2]));
                    case REQUEST_CHANGE_PASSWORD -> this.getProvider().changePassword(d[1], d[2]);
                    case REQUEST_CHANGE_DISPLAY_NAME -> this.getProvider().changeDisplayName(d[1], d[2]);
                }
            ...
            this.getTinyRabbitListener().callback((request -> {
                ...
                switch (BankCalls.valueOf(request.getKey().toUpperCase())) {
                    case REQUEST_CREATE_ACCOUNT:
                        this.getProvider().createBankAccount(d[1], d[2], (password, account) -> {
                            request.answer(BankCalls.CALLBACK_CREATE_ACCOUNT.name(), password, account);
                        });
                        break;
                    case REQUEST_GET_BANK_ACCOUNT:
                        if (this.getProvider().bankAccounts.containsKey(d[1])) {
                            final BankAccount bankAccount = this.getProvider().getAccount(d[1]);
                    ...
                    case REQUEST_BANKACCOUNTS_BY_PLAYER:
                        request.answer(BankCalls.CALLBACK_BANKACCOUNTS_BY_PLAYER.name(), this.getProvider().getAccountsByPlayer(d[1]));
                        break;
                }
